    ...
    for (int step = 0; step < nsteps; ++step)
    {
        ...
        case Render:
            if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots)
            {
                ...
                if (m_showBackground)
                {
                    ...
                    if (!m_hideEdgeBackground || x >= 8)
                    {
                        ...
                        //const Byte attribute = m_bus.read(addr);
                        const Byte attribute = m_bus.read_2000_2fff(addr);
                        const int shift = ((m_dataAddress >> 4) & 4) | (m_dataAddress & 2);
                ...
                {
                    for (Byte idx = 0; idx < m_scanlineSpritesNum; ++idx)
                    {
                        const Byte i = m_scanlineSprites[idx];
                        const Byte spr_x = m_spriteMemory[i * 4 + 3];

                        if (0 > x - spr_x || x - spr_x >= 8)
                            continue;

                        const Byte spr_y = m_spriteMemory[i * 4 + 0] + 1, tile = m_spriteMemory[i * 4 + 1],
                            attribute = m_spriteMemory[i * 4 + 2];

                        const int length = (m_longSprites) ? 16 : 8;
                ...
                m_screen.setPixel(m_scanlineOffset + x, m_bus.readPalette(paletteAddr));
            }
            ...
            if (m_cycle >= ScanlineEndCycle)
            {
                ...
                {
                    const int diff = (m_scanline - m_spriteMemory[i * 4]);
                    if (0 <= diff && diff < range)
                    {
                        ...
                        m_scanlineSprites[m_scanlineSpritesNum++] = i;
                        ++j;
                    }
                }

                m_scanline++;
                m_scanlineOffset += 360;
                m_cycle = 0;
            }
...
void PPU::doDMA(const Byte* page_ptr)
{
    ...
    if (m_spriteDataAddress)
        std::memcpy(m_spriteMemory, page_ptr + (256 - m_spriteDataAddress), m_spriteDataAddress);
}
