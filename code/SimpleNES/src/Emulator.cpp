...
void Emulator::run(std::string rom_path)
{
    ...
    m_emulatorScreen.create(NESVideoWidth, NESVideoHeight, 0.0f, 0x000000ff);

    m_lastWakeup  = rt_timer_get_ms(); // high_resolution_clock::now();
    ...
    while (true)
    {
        ...
        // if (focus && !pause)
        {
            const auto now  = rt_timer_get_ms(); // high_resolution_clock::now();
            m_elapsedTime  += now - m_lastWakeup;
            m_lastWakeup    = now;

            //while (m_elapsedTime > cpu_clock_period_ns)
            while (rt_timer_get_ms() < now + 100)
            {
                m_ppu.step(3);
                m_cpu.step();
                m_apu.step();
