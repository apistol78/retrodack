 ...
 */
static uint8_t stringDescriptors[][26]=
{
	/*! index 0 - language */
	{
		4,              // length
 ...
 */
static uint8_t usb_mass_send_descriptor(uint8_t type, uint8_t index, uint16_t reqLen)
{
	// maximum length 255
	...
	switch (type)
	{
		...
		case 1:
		{
			...
			return max3420_write_endpoint_0(deviceDescriptor, (rl < al) ? rl : al);
		}
		...
		case 2:
		{
			...
			return max3420_write_endpoint_0(configurationDescriptor, (rl < al) ? rl : al);
		}
		...
		case 3:
		{
			...
			uint8_t al = *(uint8_t*)(&stringDescriptors[index][0]);
			return max3420_write_endpoint_0(&stringDescriptors[index][0], (rl < al) ? rl : al);
		}
 ...
 */
static void usb_mass_process_standard_request(struct usbSetupPacket *setup)
{
	...
	switch (setup->requestType & 0x0F) 
	{
		case REQUEST_RECP_DEVICE:
			...
			switch (setup->request) 
			{
				case STANDARD_REQUEST_GET_STATUS:
					/* send deviceStatus */
					if (max3420_write_endpoint_0((uint8_t *)(&deviceStatus), 2) == 0)
					{
						max3420_ack_status();
					}
					else
					{
						max3420_stall_endpoint(0);
					}
					break;
				...
				case STANDARD_REQUEST_SET_ADDRESS:
					 ...
					 */
					max3420_set_device_address(setup->value_lo);
					break;
				...
				case STANDARD_REQUEST_GET_DESCRIPTOR:
					 ...
					 */
					if (usb_mass_send_descriptor(setup->value_hi, setup->value_lo, (uint8_t)setup->length) == 0)
					{
						max3420_ack_status();
					}
					else
				...
				case STANDARD_REQUEST_GET_CONFIGURATION:
					...
					if (max3420_write_endpoint_0((uint8_t*)&deviceConfigState, 1) == 0)
					{
						max3420_ack_status();
					}
					else
		...
		case REQUEST_RECP_INTERFACE:
			...
			switch (setup->request) 
			{
				...
				{
					uint8_t status[] = {0, 0,};
					if (max3420_write_endpoint_0(status, 2) == 0)
					{
						max3420_ack_status();
					}
					else
				...
				{
					uint8_t interface = 0;
					if(max3420_write_endpoint_0(&interface, 1) == 0)
					{
						max3420_ack_status();
					}
					else
		...
		case REQUEST_RECP_ENDPOINT:
			...
			{
				/* get endpoint status:
				 * get endpoint status by calling max3420_is_endpoint_stalled. Mask everything except endpoint number
				 * in function call.
				...
				case STANDARD_REQUEST_GET_STATUS:
				{
					...
					if (max3420_is_endpoint_stalled((uint8_t)setup->index & 0x0F))
					{
						status = 0x0001;
					}
					if (max3420_write_endpoint_0((uint8_t *)&status, 2) == 0)
					{
						max3420_ack_status();
					}
					else
				 ...
				 */
				case STANDARD_REQUEST_CLEAR_FEATURE:
					max3420_clear_stall_endpoint((uint8_t)setup->index & 0x0F);
					max3420_ack_status();
					break;
					
				case STANDARD_REQUEST_SET_FEATURE:
					max3420_stall_endpoint((uint8_t)setup->index & 0x0F);
					max3420_ack_status();
					break;
 ...
 *  setup:      Pointer to setup packet struct.
 */
static void usb_mass_process_class_request(struct usbSetupPacket *setup)
{
	...
	switch (setup->request)
	{
		...
		{
			uint8_t maxLun = 0;
			if (max3420_write_endpoint_0(&maxLun, 1) == 0)
			{
				max3420_ack_status();
			}
			else
 ...
 */
static void usb_mass_pad_bytes_in(uint32_t bytes)
{
	...
	while (bytes)
	{
		...
		{
			if (max3420_write_endpoint_2(foo, EP_BUFFER_SIZE) != 0)
			{
		...
		{
			max3420_write_endpoint_2(foo, (uint8_t)bytes);
			break;
		}
	}
}
 ...
 */
static void usb_mass_pad_bytes_out(uint32_t bytes)
{
	uint8_t foo[EP_BUFFER_SIZE];
	...
	while (bytes)
	{
		if (bytes > EP_BUFFER_SIZE)
		{
			if (max3420_read_endpoint_1(foo) != EP_BUFFER_SIZE)
			{
				break;      // endpoint buffer not available and another USB event active
			}
			bytes -= EP_BUFFER_SIZE;
 ...
 */
static uint8_t usb_mass_bulk_command_transport()
{
	uint8_t buffer[EP_BUFFER_SIZE];
	
	/* get new CBW */
	if (max3420_read_endpoint_1(buffer) != 31)
	{                         
 ...
 */
static void usb_mass_bulk_status_transport(enum blkCommandStatus stat)
{
	if (stat == COMMAND_STATUS_FAIL)
		printf("usb_mass_bulk_status_transport, COMMAND_STATUS_FAIL\n");
	else if (stat == PHASE_ERROR)
		printf("usb_mass_bulk_status_transport, PHASE_ERROR\n");

	blkx.stat.tag = blkx.cmd.tag;
	blkx.stat.status = stat;
	
	max3420_write_endpoint_2((uint8_t *)&blkx.stat, sizeof blkx.stat);  // write CSW to IN endpoint
}
 ...
 */
static void usb_mass_handle_bulk_transport_error(uint8_t scsiCmdStat)
{
	...
	/* data transport device -> host */
	{
		...
		/* case 7 or 8 */
		{
			if (blkx.bytesIn)
			{
				usb_mass_pad_bytes_in(blkx.bytesIn);
			}
			usb_mass_bulk_status_transport(PHASE_ERROR);
		}
		...
		/* case 4 or 5 */
		{
			...
			if (scsiCmdStat == 0)
			{
				usb_mass_bulk_status_transport(COMMAND_STATUS_PASS);
			}
			else
			{
				usb_mass_bulk_status_transport(COMMAND_STATUS_FAIL);
			}
		}
		else
		{
	...
	/* data transport host -> device */
	{
		...
		/* case 10 or 13 */
		{
			if (blkx.bytesOut)
			{
				usb_mass_pad_bytes_out(blkx.bytesOut);
			}
			usb_mass_bulk_status_transport(PHASE_ERROR);
		}
 ...
 */
uint8_t usb_mass_write_bulk_in(const uint8_t *buffer, uint8_t length)
{
	/* check if host is expecting less than what is being attempted to be sent */
	if (length > blkx.bytesIn)
	{
		printf("[ERROR] usb_mass_write_bulk_in 1\n");
		blkx.xferError = 1;
		return 1;
	}
	/* check if write fails */
	if (max3420_write_endpoint_2(buffer, length) != 0)
	{
		printf("[ERROR] usb_mass_write_bulk_in 2\n");
		return 1;
	}
 ...
 */
uint8_t usb_mass_read_bulk_out(uint8_t *buffer)
{
	uint8_t length;
	length = max3420_read_endpoint_1(buffer);
 ...
 */
void usb_mass_process_bus_reset()
{
	scsi_reset();
}
 ...
 */
void usb_mass_process_setup_packet()
{
	struct usbSetupPacket setup;
	
	if (max3420_get_setup_packet((uint8_t *)&setup) != 0)
	{
		printf("ERROR: Unable to get setup packet!\n");
		max3420_stall_endpoint(0);
		return;
	}
	...
	switch (setup.requestType & 0x60)
	{
		case REQUEST_TYPE_STANDARD:
			// printf("REQUEST_TYPE_STANDARD\n");
			usb_mass_process_standard_request(&setup);
			break;
			
		case REQUEST_TYPE_CLASS:
			// printf("REQUEST_TYPE_CLASS\n");
			usb_mass_process_class_request(&setup);
			break;
			
		case REQUEST_TYPE_VENDOR:
			// printf("REQUEST_TYPE_VENDOR\n");
			max3420_stall_endpoint(0);
			break;
 ...
 */
void usb_mass_process_bulk_out_transaction()
{
	// printf("... usb_mass_process_bulk_out_transaction\n");

	/*  SCSI Transaction struct used by USB library to communicate with SCSI library */
	scsiTransaction_t scsix =
	{
		.cdb = blkx.cmd.cb,
		.write = &usb_mass_write_bulk_in,
		.read = &usb_mass_read_bulk_out,
	};
	...
	/* command transport */
	if (usb_mass_bulk_command_transport() != 0)
	{
		printf("... usb_mass_process_bulk_out_transaction, usb_mass_bulk_command_transport failed\n");
		max3420_stall_endpoint(1);
		return;
	}
	...
	if ((blkx.cmd.flags & 0x80) && blkx.cmd.dataTransferLength > 0)
	{
		...
		// printf("usb_mass_process_bulk_out_transaction, bytesIn %d from host\n", blkx.bytesIn);
	}
	else if (blkx.cmd.dataTransferLength > 0)
	{
		/* data transport host -> device */
		blkx.bytesOut = blkx.cmd.dataTransferLength;
		// printf("usb_mass_process_bulk_out_transaction, bytesOut %d from device\n", blkx.bytesOut);
	}
	
	/* process SCSI command */
	scsi_process_command(&scsix);
	
	/* check data transport */
	if (blkx.xferError || blkx.bytesIn || blkx.bytesOut)
	{
		/* bulk transfer data management error has occurred */
		printf("... usb_mass_process_bulk_out_transaction error, xferError = %d, bytesIn = %d, bytesOut = %d\n", blkx.xferError, blkx.bytesIn, blkx.bytesOut);
		usb_mass_handle_bulk_transport_error(scsix.cmdStatus);
	}
	else
	{
		...
		{
			// printf("... usb_mass_process_bulk_out_transaction, COMMAND_STATUS_PASS\n");
			usb_mass_bulk_status_transport(COMMAND_STATUS_PASS);
		}
		else
		{
			printf("... usb_mass_process_bulk_out_transaction, COMMAND_STATUS_FAIL\n");
			usb_mass_bulk_status_transport(COMMAND_STATUS_FAIL);
		}
	}
}
